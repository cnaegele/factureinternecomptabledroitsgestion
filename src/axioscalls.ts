...
export async function getComptableGestion(idUO: number): Promise<ApiResponseEmpS> {
    const page: string = '/goeland/gestion_spec/afffactureint_comptabledroitgestion/axios/afffactureint_comptablegestion_liste.php'
    const url: string = `${server}${page}`
...
export async function getEmployesListe(critere: string, bretInactif: boolean): Promise<EmployeChoix[]> {
    const page: string = '/goeland/gestion_spec//afffactureint_comptabledroitgestion/axios/employes_liste.php'
    const url: string = `${server}${page}`
...
export async function supprimeDroitComptableGestion(idOrgunit: number, idEmploye: number): Promise<ApiResponseMessage> {
    const odata = {
        idorgunit: idOrgunit,
        idemploye: idEmploye
    }
    const jdata: string = JSON.stringify(odata)
    const page: string = '/goeland/gestion_spec//afffactureint_comptabledroitgestion/axios/afffactureint_comptablegestion_supprime.php'
    const url: string = `${server}${page}`
    try {
...
export async function sauveDroitComptableGestion(idOrgunit: number, idEmploye: number): Promise<ApiResponseMessage> {
    const odata = {
        idorgunit: idOrgunit,
        idemploye: idEmploye
    }
    const jdata: string = JSON.stringify(odata)
    const page: string = '/goeland/gestion_spec//afffactureint_comptabledroitgestion/axios/afffactureint_comptablegestion_sauve.php'
    const url: string = `${server}${page}`
    try {
        const response: ApiResponseMessage = await axios.post(url, jdata, {
            headers: {
